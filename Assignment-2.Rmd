...
Train <-DF_n[Index_train,]
Test<-DF_n[-Index_train,]
...
model
```
2.) The perfect balance for k would be 6.
...
```
```{r}
Train_Predictors1 <- Train[,2:3]
Test_Predictors1 <- Test[,2:3]

Train_labels1 <- Train[,1]
Test_labels1 <- Test[,1]

Predict_Test_labels1 <- knn(Train_Predictors1, Test_Predictors1, cl= Train_labels1, k=7)

head(Predict_Test_labels1)
```

```{r}
CrossTable(x=Test_labels1, y=Predict_Test_labels1, prop.chisq = FALSE)
```
The difference between adding a validation is fairly minimal. The K was only different by one. There is more false negatives which is more harmful. However, there is less false positives. This is is less harmful but it is still imported to have less false positives.I think the differences is the the k is higher more data points are included making it easier more produce.
